...
def main():
    ...
    print(f"Building prototypes for {num_known_classes} known classes ...")
    ...
    for c in range(num_known_classes):
        ...
        if count == 0:
            warnings.warn(f"Class {c} has no samples in {args.split} split — prototype will be zero vector")
            continue
        if count == 1:
            warnings.warn(f"Class {c} has only 1 sample — prototype may be unreliable")
